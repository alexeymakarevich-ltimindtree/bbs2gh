...
Write-VerboseLog "gh version: $((& gh --version | Select-Object -First 1))"
& gh bbs2gh --version *> $null
if ($LASTEXITCODE -ne 0) {
    Write-Host "[ERROR] Required gh extension 'gh-bbs2gh' is not installed. Install with: gh extension install github/gh-bbs2gh" -ForegroundColor Red
    exit 1
}
Write-VerboseLog "gh bbs2gh version: $((& gh bbs2gh --version 2>$null | Select-Object -First 1))"
...
if ($migrated.Count -eq 0) {
    Write-Host "::error::No repositories were migrated successfully (0 succeeded out of $totalRepos)."
    foreach ($f in $failed) { Write-Host "::error::Failed: $f" }
    exit 1
}
elseif ($failed.Count -eq 0) {
    Write-Host "::notice::All $totalRepos repositories migrated successfully"
    exit 0
}
else {
    Write-Host "::warning::Migration completed with partial success: $($migrated.Count) succeeded, $($failed.Count) failed out of $totalRepos total"
    foreach ($f in $failed) { Write-Host "::warning::Failed: $f" }
    exit 0
}
