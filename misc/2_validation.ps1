...
function Curl-Json([string]$Url) {
  return Invoke-RestMethod -Headers (Get-AuthHeader) -Uri $Url -Method Get @script:RestArgs
}
...
if ($script:DisableSslVerify) {
  ...
  try {
    ...
    if ("$_" -match 'SSL|certificate|trust|SEC_ERROR|self-signed|CERT_') {
      Write-Error "[ERROR] TLS/SSL certificate validation failed for $BASE_URL. Detail: $_. If this host uses a self-signed or internal CA certificate intentionally, re-run with BBS_DISABLE_SSL_VERIFY=Y."
      exit 1
    }
  }
}
...
foreach ($line in $lines) {
  ...
  $header_line = "ℹ️  [$([DateTime]::UtcNow.ToString('yyyy-MM-ddTHH:mm:ssZ'))] Validating: $bbsProjectKey/$bbsRepoSlug -> $ghOrg/$ghRepo"
  ...
  if ($branchCountOk -eq 'true') {
    ("✅ Branch Count MATCHED | BBS=$bbsBranchCount GitHub=$ghBranchCount") | Tee-Object -FilePath $LOG_FILE -Append
  } else {
    ("❌ Branch Count MISMATCH | BBS=$bbsBranchCount GitHub=$ghBranchCount") | Tee-Object -FilePath $LOG_FILE -Append
  }
  ...
  if ($missingInGH) {
    ("❌ Branches missing in GitHub: $((@($missingInGH) -join ' '))") | Tee-Object -FilePath $LOG_FILE -Append
  } else {
    ("✅ No branches missing in GitHub") | Tee-Object -FilePath $LOG_FILE -Append
  }
  if ($missingInBBS) {
    ("❌ Extra branches in GitHub (not in Bitbucket): $((@($missingInBBS) -join ' '))") | Tee-Object -FilePath $LOG_FILE -Append
  } else {
    ("✅ No extra branches found") | Tee-Object -FilePath $LOG_FILE -Append
  }
  ...
  if ($COMMIT_CHECK -ne 'true') {
    ...
    if ($bbsBranchCount -gt 10 -or $ghBranchCount -gt 10) {
      ("ℹ️ Commit validation running only for first $($branchesToCheck.Count) branches (default branch first, max 10)") | Tee-Object -FilePath $LOG_FILE -Append
    } elseif ($branchesToCheck.Count -gt 0) {
      ("ℹ️ Commit validation covering $($branchesToCheck.Count) branch(es) (default branch first, max 10)") | Tee-Object -FilePath $LOG_FILE -Append
    } else {
      ("ℹ️ Could not determine any branch for commit/SHA check") | Tee-Object -FilePath $LOG_FILE -Append
    }
    ...
    if ($branchesToCheck.Count -gt 0) {
      ...
      foreach ($br in $branchesToCheck) {
        if ($ghBranches -notcontains $br) {
          ("Branch '$br': missing in GitHub branches list ❌ Not Matching") | Tee-Object -FilePath $LOG_FILE -Append
          $commitsMatchAll = 'false'; $shasMatchAll = 'false'; continue
        }
        if ($bbsBranches -notcontains $br) {
          ("Branch '$br': missing in Bitbucket branches list ❌ Not Matching") | Tee-Object -FilePath $LOG_FILE -Append
          $commitsMatchAll = 'false'; $shasMatchAll = 'false'; continue
        }
        ...
        ("Branch '$br': BBS Commits=$bbsCount | GitHub Commits=$ghCount | $(Status-Marker $countOk)") | Tee-Object -FilePath $LOG_FILE -Append
        ("Branch '$br': BBS SHA=$bbsSha | GitHub SHA=$ghSha | $(Status-Marker $shaOk)") | Tee-Object -FilePath $LOG_FILE -Append
      }
    }
  }
  ...
  ("✅ Validation completed for: $ghOrg/$ghRepo") | Tee-Object -FilePath $LOG_FILE -Append
