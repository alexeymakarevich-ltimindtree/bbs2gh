...
if ($MAX_CONCURRENT -gt 5) {
  Write-Host "`e[31m[ERROR] Maximum concurrent migrations ($MAX_CONCURRENT) exceeds the allowed limit of 5.`e[0m"
  exit 1
}
...
function Get-BbsInstallPath {
  if ($env:BITBUCKET_HOME -and (Test-Path -LiteralPath $env:BITBUCKET_HOME -PathType Container)) {
    Write-Host "`e[32m[OK] Bitbucket Server home found via BITBUCKET_HOME: $($env:BITBUCKET_HOME)`e[0m"
    return
  }
  $detected = $null
  try {
  ...
  if ($detected) {
    ...
    Write-Host "`e[32m[OK] Bitbucket Server home auto-detected from running process: $detected`e[0m"
    return
  }
  foreach ($p in @('/var/atlassian/application-data/bitbucket','/opt/atlassian/bitbucket','C:\Atlassian\ApplicationData\Bitbucket','C:\Program Files\Atlassian\Bitbucket')) {
    if (Test-Path -LiteralPath $p -PathType Container) {
      ...
      Write-Host "`e[32m[OK] Bitbucket Server found at default location: $p`e[0m"
      return
    }
  }
  ...
  if ($launcher) {
    ...
    Write-Host "`e[32m[OK] Bitbucket Server launcher found on PATH: $($launcher.Source) (home: $bbsHome)`e[0m"
    return
  }
  Write-Host "`e[33m[WARNING] Bitbucket Server install not found locally (checked BITBUCKET_HOME, running process, default dirs, PATH). Continuing (remote/SSH migration does not require a local install).`e[0m"
}
Get-BbsInstallPath
  ...
  while (($JOB_JOBS.Keys.Count -lt $MAX_CONCURRENT) -and ($QUEUE.Count -gt 0)) {
    ...
    $job = Start-Job -ScriptBlock {
      ...
      try {
        ...
        $tlsArgs = @()
        if ($env:BBS_DISABLE_SSL_VERIFY -match '^(?i:y|yes|true|1)$') { $tlsArgs = @('--no-ssl-verify') }
...
if ($MIGRATED.Count -eq 0) {
  Write-Host "::error::No repositories were migrated successfully (0 succeeded out of $total_repos)."
